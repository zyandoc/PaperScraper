...
from ntscraper_local import NTScraper
...
def get_filtered_tweets(link, target_usernames, max_results=50):
    scraper = NTScraper()
    normalized_users = {user.lstrip("@").lower() for user in target_usernames}
    results = scraper.search(link, mode="live", limit=max_results)

    filtered_tweets = [
        tweet for tweet in results["tweets"]
        if tweet["user"]["username"].lower() in normalized_users
    ]
    return filtered_tweets

if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python nitter.py <url> <comma-separated-usernames>")
        sys.exit(1)

    url = sys.argv[1]
    usernames = sys.argv[2].split(",")

    tweets = get_filtered_tweets(url, usernames, max_results=50)

    if not tweets:
        print("No matching tweets found.")
    else:
        for tweet in tweets:
            print(f"[{tweet['time']}] @{tweet['user']['username']}:\n{tweet['text']}\n")
